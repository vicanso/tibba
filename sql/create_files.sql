CREATE TABLE `files` (
    ...
    -- 图片特有属性
    `image_width` INTEGER NOT NULL DEFAULT -1 COMMENT '图片宽度',
    `image_height` INTEGER NOT NULL DEFAULT -1 COMMENT '图片高度',
    
    -- 通用元数据
    `metadata` JSON NOT NULL DEFAULT (JSON_OBJECT()) COMMENT '存储其他元数据信息',
