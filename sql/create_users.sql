CREATE TABLE `users` (
  ...
  `totp_secret` TEXT DEFAULT NULL COMMENT 'TOTP 密钥，AES-256-GCM 加密后 base64；NULL 表示未注册 2FA',
  `totp_enabled_at` DATETIME DEFAULT NULL COMMENT '2FA 激活时间；NULL 表示未启用',
  `totp_recovery_codes` JSON DEFAULT NULL COMMENT '一次性恢复码的 SHA-256(base64) 数组',
