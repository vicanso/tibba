...
use crate::{LoginParams, user_agent_of};
use axum::Json;
...
/// `POST /login/jwt` —— 同 `/login` 校验链，成功签 access + 发 refresh。
pub(crate) async fn login_jwt(
    ...
    // 5. 发 opaque refresh，Redis 持有 (user_id, account)
    ...
    // 6. last_login_at + audit（与 session login 平行；失败仅日志，不阻塞）
    ...
    let _ = AuditLogModel::new()
        .log(
            state.pool,
            AuditLogParams::new("user.login_jwt")
                .with_user(user.id)
